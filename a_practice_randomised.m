function a_practice_randomised(subNo)
...
try        
    ...
    % screen set up:
    %---------------------     
    ptb_screen_setup(1) % prepare MATLAB for ptb
    ...
    for trial = 1:80;
        ...
        elseif trial == 41;
            L_ex22_auditory_sweep(subNo, w, img_tex, img_loc);
            message = strcat('Click to continue when ready...');  % write block message
            ptb_onscreen_text (w, message);  % present block message - wait for click 
        elseif trial == 57
            L_ex22_auditory_sweep(subNo-1, w, img_tex, img_loc);   
        end
